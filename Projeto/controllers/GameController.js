...
class GameController {
  ...
  async ErrorGame(user, moneyTotal, ajuda) {
      ...
      var status = await statusController.GetStatusByUserId(user.UserId);
        ...
        //console.log(status);
				...
				status = {
									...
									TotalPremio:status.TotalPremio + moneyTotal,
									NumAlternativasEliminadas:status.NumAlternativasEliminadas + (ajuda == 1) ? 0 : 2,
									NumDerrotasErro:status.NumDerrotasErro + 1,
									NumDerrotasParada:status.NumDerrotasParada,
									NumContribuicao:0,
									NivelUsuario:'0',
									UserId:user.UserId
								 }
  ...
  async WinnerGame(user, moneyTotal, ajuda) {
